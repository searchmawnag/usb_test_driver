...
// global mutex
static DEFINE_MUTEX(usbtest_mutex);
...
static int uvc_get_video_ctrl(struct uvc_streaming *stream, struct uvc_streaming_control *ctrl, int probe, __u8 query)
{
	__u8 *data;
	__u16 size;
	int ret;
	int uvc_timeout_param = 0;
 ...
 */
int usb_uvc_cmd_test(struct uvc_streaming *stream)
{
	...
	if (stream->nformats == 0) {
		printk(KERN_INFO, "No supported video formats found.\n");
		return -EINVAL;
	}
...
static int test_ctrl_queue(struct usbtest_dev *dev, struct usbtest_param_32 *param)
{
...
static int unlink_queued(struct usbtest_dev *dev, int pipe, unsigned num, unsigned size)
{
 ...
 */
static int ctrl_out(struct usbtest_dev *dev, unsigned count, unsigned length, unsigned vary, unsigned offset)
{
	unsigned		i, j, len;
	int			retval;
	u8			*buf;
...
static int test_queue(struct usbtest_dev *dev, struct usbtest_param_32 *param, int pipe, struct usb_endpoint_descriptor *desc, unsigned offset)
{
...
/**************************  Standard File Operation API for Driver *************************/
...
static ssize_t usbtest_write(struct file *file, const char *user_buffer, size_t count, loff_t *ppos)
{
	//struct usb_skel *dev;
	printk("usbtest write\n");
	return 0;
}


struct usb_driver usbtest_driver;
static ssize_t usbtest_open(struct inode* inode, struct file *file)
{
	struct usbtest_dev *dev = NULL;
	int subminor;//struct usb_host_interface *iface_desc;
	struct usb_interface *interface;
	
	printk("usbtest open\n");
	mutex_lock(&usbtest_mutex);
	subminor = iminor(inode);
	
	interface = usb_find_interface(&usbtest_driver, subminor);
	if (!interface) {
		mutex_unlock(&usbtest_mutex);
		printk(KERN_ERR "USBTest: %s - error, can't find device for minor %d\n",
		       __func__, subminor);
		return -ENODEV;
	}
...
static ssize_t usbtest_release(struct inode* inode, struct file *file)
{
	printk("usbtest release\n");
	return 0;
}


static ssize_t usbtest_flush(struct file *file, fl_owner_t id)
{
	return 0;
}

/* Run tests. */
static int usbtest_do_ioctl(struct usbtest_dev* dev, struct usb_interface *intf, struct usbtest_param_32 *param)
{
	...
	case 28:
		...
		break;
	default:
		dev_warn(&intf->dev, "TEST num is err\n");
		break;
...
static int usbtest_ioctl(struct file* filep, unsigned int code, void* buf)
{
	struct usbtest_dev	*dev;// = usb_get_intfdata(intf);
	struct usb_interface* intf;
	struct usbtest_param_64 *param_64 =NULL;
	struct usbtest_param_32 temp;
	struct usbtest_param_32 *param_32 =NULL;
	...
	/* malloc for param  */
	param_64 = kzalloc(sizeof(struct usbtest_param_64), GFP_KERNEL);
	if(! param_64)
		goto EXIT;	
	param_32 = kzalloc(sizeof(struct usbtest_param_32), GFP_KERNEL);
	if(! param_32) {
		kfree(param_64);
		goto EXIT;
	}
	...
	case USBTEST_REQUEST_64:
		...
		if( copy_from_user(param_64, buf, sizeof(struct usbtest_param_64)))
			return -EFAULT;
	...
	case USBTEST_REQUEST_32:
		/* Copy data from userspace to kernel */
		if( copy_from_user(param_32, buf, sizeof(struct usbtest_param_32)))
			return -EFAULT;
		break;
	default:
		retval = -EOPNOTSUPP;
	...
	printk("usbtest ioctl 3\n");
	ktime_get_ts64(&start);
	
	retval = usbtest_do_ioctl(dev, dev->intf, param_32);
	if (retval < 0)  {
		printk("usbtest_do_ioctl err\n");
		goto free_mutex;
	}
...
#ifdef	GENERIC
static unsigned short vendor =0x2084;
module_param(vendor, ushort, 0);
MODULE_PARM_DESC(vendor, "vendor code (from usb-if), default: 0x2084");

static unsigned short product=0x0505;
module_param(product, ushort, 0);
MODULE_PARM_DESC(product, "product code (from vendor), default:0x0505");
#endif


/*******************************************************
   Standard File Operations pointer for device driver 
********************************************************/
static const struct file_operations usbtest_fops = {
	...
	//.llseek =	usbtest_llseek,
};


#define USBTEST_DEV_MINOR_BASE			194
...
static struct usb_class_driver usbtest_class = {
	.name =		"usbtest",
	.fops =		&usbtest_fops,
	.minor_base = USBTEST_DEV_MINOR_BASE,
};


static int usbtest_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	struct usb_device	*udev;
	...
	dev_info(&intf->dev,
		 "USBtest driver class device now attached to USBtest%d",
		 intf->minor);
